...
(async () => {
  ...
  window.max.bindInlet("set_mediadevice", async function (deviceLabel) {
    ...
    if (!devices.length) {
      window.max.outlet(
        "error",
        `No video input device: "${deviceLabel}" exists.`
      );
      return;
    }
  ...
  const getVideoDevicesForMax = () => {
    getMediaDevices()
      .then((devices) => {
        ...
        window.max.outlet.apply(
          window.max,
          ["mediadevices"].concat(mediadevices)
        );
      })
      .catch((err) => {
  ...
  const setRunningMode = async (running_mode) => {
    ...
    switch (running_mode) {
      ...
      default:
        window.max.outlet(
          "error",
          `No running mode: "${running_mode}" exists.`
        );
        return;
    }
  };
    ...
    if (results.handednesses) {
      for (const hand of results.handednesses) {
        Object.values(HAND_LANDMARKS).forEach(([landmark, index]) => {
          try {
            const handIndex =
              results.handednesses.length > 1 ? Number(hand[0].index) : 0;
            ...
            if (results.gestures.length > 0) {
              let categoryName = results.gestures[handIndex][0].categoryName;
              output[handName]["Gestures"] = output[handName]["Gestures"] || {};
              if (categoryName === "None") {
                const pinkyTipY = results.landmarks[handIndex][20]["y"];
                const pinkyPipY = results.landmarks[handIndex][18]["y"];
                if (pinkyTipY < pinkyPipY) {
                  // y coordinates are flipped
                  categoryName = "Four";
                } else {
                  const ringTipY = results.landmarks[handIndex][16]["y"];
                  const ringPipY = results.landmarks[handIndex][14]["y"];
                  if (ringTipY < ringPipY) {
                    categoryName = "Three";
                  }
                }
              }
              output[handName]["Gestures"][categoryName] =
                results.gestures[handIndex][0].score;
            }
          } catch (e) {
            console.error(e);
          }
      ...
      for (const landmarks of results.landmarks) {
        if (drawHands) {
          ...
          drawingUtils.drawLandmarks(landmarks, {
            ...
            radius: (data) => {
              return DrawingUtils.lerp(data.from.z, -0.15, 0.1, 2, 1);
            },
          });
        }
      }
